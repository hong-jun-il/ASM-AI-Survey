...
export function Question({ q, value, counts, onChange }: QuestionProps) {
  ...
  const isSelected = (optId: string) =>
    q.type === "multi"
      ? ((value as string[] | undefined) ?? []).includes(optId)
      : value === optId;

  const { opts, denom, respondents, voted } = tallyQuestion(q, value, counts);
  const stat = new Map(
    opts.map((o) => [
      o.id,
      { pct: denom ? (o.tally / denom) * 100 : 0, tally: o.tally },
    ]),
  );
  ...
  if (q.groups) {
    ...
    body = (
      <div className="flex flex-col gap-2.5">{opts.map(renderOption)}</div>
    );
  }
      ...
      <div className="mb-4 flex items-baseline gap-3">
        ...
        <div className="flex flex-col gap-0.75">
          ...
          <span className="text-[13px] text-faint">
            {voted ? "다시 누르면 취소돼요" : q.hint}
          </span>
        </div>
      </div>
      {body}
      {voted && (
        <div className="mt-3 text-[12.5px] text-faint tabular-nums">
          {respondents.toLocaleString()}명 응답 · 당신 포함
        </div>
      )}
    </section>
  );
